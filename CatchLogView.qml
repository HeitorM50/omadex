...
Column {
  ...
  Text {
    ...
    text: root.rows.length === 0
          ...
          : root.rows.length + (root.rows.length === 1 ? " criado" : " criados")
  ...
  Text {
    ...
    textFormat: Text.PlainText
    text: "Cada Pokémon que chocar aparece aqui, e continua aqui depois de graduar."
  ...
  ListView {
    ...
    delegate: Item {
        ...
        Text {
          ...
          text: {
            ...
            if (row.modelData.lineLength > 1)
              bits.push("estágio " + (row.modelData.finalStage + 1)
      ...
      Text {
        ...
        text: row.modelData.current ? "agora" : Collection.formatDate(row.modelData.hatchedAt)
